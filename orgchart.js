...
(function (factory) {
    ...
    OrgChart.prototype = {
        ...
        init: function (opts) {
            ...
            let parent = $chart.parent().parent();
            let width = parent.outerWidth();

            $(window).resize(() => {
                    if (width !== 0 && width && Math.abs(parent.outerWidth()-width)>20) {
                        location.reload()
                    }
                }
            );
            this.setChartScale($chart, 1);
            if ($chart.outerWidth() > $(window).width()) {
                var size = $chart.outerWidth()
                $chart.css({'width': $(window).width() - 50, 'transform-origin': 'top left'});
                this.setChartScale($chart, $chart.outerWidth() / size)
            }
            return this;
        },
        //
        ...
        createNode: function (data) {
            ...
            } else if (opts.disableRelationship) {
            } else {
        ...
        buildHierarchy: function ($appendTo, data) {
            ...
            if (Object.keys(data).length > 2) {
                ...
                if (opts.verticalLevel && level >= opts.verticalLevel) {
                    ...
                    $nodeWrapper = $('<table>');
                    var td_table = $('<td' + (hasChildren ? ' colspan="' + childrenData.length * 2 + '"' : '') + '></td>').append($nodeDiv);
                    var tr_table = $('<tr/>').append(td_table);
                    $nodeWrapper.append(tr_table);
            ...
            if (hasChildren) {
                ...
                if (isVerticalLayer) {
                    ...
                    if (data.className === "line-node") {
                        var $upperLines = $('<tr class="lines long-line' + isHidden + '"><td colspan="' + childrenData.length * 2 + '"><div class="downLine"></div></td></tr>');
                    } else {
        ...
        //
        export: async function (exportFilename, exportFileextension) {
            var that = this;
            ...
            var $chartContainer = this.$chartContainer;
            var $mask = $chartContainer.find('.sea');
            if (!$mask.length) {
                $chartContainer.append('<div class="sea">\n' +
                    ...
                    '        </div>\n' +
                    '    </div>\n' +
                    '    <h1>Please wait, generating PDF and PNG</h1>'+
                    '    <h6>This process can take a few minutes, if you dont use chrome, you might have bad results</h6>'+
                    '</div>');
            } else {
            ...
            sourceChart.parentElement.style.position = "absolute";
            sourceChart.parentElement.style.left = "0";
            sourceChart.parentElement.style.top = "0";
            sourceChart.parentElement.style.width = "80vw";
            sourceChart.parentElement.style.height = "300vh";

            await html2canvas(sourceChart, {
                allowTaint: true,
                useCORS: true,
                ...
                .then(function (canvas) {
                        ...
                        } else if (exportFileextension.toLowerCase() === 'png') {
                            that.exportPNG(canvas, exportFilename);
                        }else{
                            that.exportPDF(canvas, exportFilename);
                            that.exportPNG(canvas, exportFilename);
                        }
                        // remove style and let the user enjoy the submarine ;)
                        let sub = $chartContainer.find('.sea');
                        sub.addClass('hidden');
                        sourceChart.parentElement.style.position = "relative";
                        sourceChart.parentElement.style.removeProperty("left");
                        sourceChart.parentElement.style.removeProperty("top");
                        sourceChart.parentElement.style.removeProperty("width");
                        sourceChart.parentElement.style.removeProperty("height");
